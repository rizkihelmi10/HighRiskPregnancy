package com.kamus.highriskpregnancy;

public class AnswerModel3 {
    private String answer1;
    private String answer2;
    private String answer2a;
    private String answer2b;
    private String answer2d;
    private String answer2e;
    private String answer3;
    private String answer4;
    private String answer4a;
    private String answer4b;
    private String answer5;
    private String answer6;
    private String answer7;
    private String answer8;
    private String answer9;
    private String answer9a;
    private String answer9b;
    private String answer10;
    private String answer11;
    private String answer12;
    private int score;

    public String getAnswer1() {
        return answer1;
    }

    public void setAnswer1(String answer1) {
        this.answer1 = answer1;
    }

    public String getAnswer2() {
        return answer2;
    }

    public void setAnswer2(String answer2) {
        this.answer2 = answer2;
    }

    public String getAnswer2a() {
        return answer2a;
    }

    public void setAnswer2a(String answer2a) {
        this.answer2a = answer2a;
    }

    public String getAnswer2b() {
        return answer2b;
    }

    public void setAnswer2b(String answer2b) {
        this.answer2b = answer2b;
    }

    public String getAnswer2d() {
        return answer2d;
    }

    public void setAnswer2d(String answer2d) {
        this.answer2d = answer2d;
    }

    public String getAnswer2e() {
        return answer2e;
    }

    public void setAnswer2e(String answer2e) {
        this.answer2e = answer2e;
    }

    public String getAnswer3() {
        return answer3;
    }

    public void setAnswer3(String answer3) {
        this.answer3 = answer3;
    }

    public String getAnswer4() {
        return answer4;
    }

    public void setAnswer4(String answer4) {
        this.answer4 = answer4;
    }

    public String getAnswer4a() {
        return answer4a;
    }

    public void setAnswer4a(String answer4a) {
        this.answer4a = answer4a;
    }

    public String getAnswer4b() {
        return answer4b;
    }

    public void setAnswer4b(String answer4b) {
        this.answer4b = answer4b;
    }

    public String getAnswer5() {
        return answer5;
    }

    public void setAnswer5(String answer5) {
        this.answer5 = answer5;
    }

    public String getAnswer6() {
        return answer6;
    }

    public void setAnswer6(String answer6) {
        this.answer6 = answer6;
    }

    public String getAnswer7() {
        return answer7;
    }

    public void setAnswer7(String answer7) {
        this.answer7 = answer7;
    }

    public String getAnswer8() {
        return answer8;
    }

    public void setAnswer8(String answer8) {
        this.answer8 = answer8;
    }

    public String getAnswer9() {
        return answer9;
    }

    public void setAnswer9(String answer9) {
        this.answer9 = answer9;
    }

    public String getAnswer9a() {
        return answer9a;
    }

    public void setAnswer9a(String answer9a) {
        this.answer9a = answer9a;
    }

    public String getAnswer9b() {
        return answer9b;
    }

    public void setAnswer9b(String answer9b) {
        this.answer9b = answer9b;
    }

    public String getAnswer10() {
        return answer10;
    }

    public void setAnswer10(String answer10) {
        this.answer10 = answer10;
    }

    public String getAnswer11() {
        return answer11;
    }

    public void setAnswer11(String answer11) {
        this.answer11 = answer11;
    }

    public String getAnswer12() {
        return answer12;
    }

    public void setAnswer12(String answer12) {
        this.answer12 = answer12;
    }

    public int getScore() {
        return score;
    }

    public void setScore(int score) {
        this.score = score;
    }
}

